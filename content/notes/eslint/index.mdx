...
_It won't do anything unless you extend some else's config file or explicitly turn on rules in your config file._
...
Tell ESLint what syntax you're using. ES3-ES5 is ESLint's default but ES6/ES2015 and beyond is modern day JavaScript. Specifying `module` is important if you use `import`/`export` and `jsx` is important if you're using React.
...
It's gets fairly involved, here's [a good article](https://stevenpetryk.com/blog/custom-eslint-rules/) on it.

## My personal setup for a NextJS Website
...
- [eslint-config-prettier](https://github.com/prettier/eslint-config-prettier) OR [eslint-plugin-prettier](https://github.com/prettier/eslint-plugin-prettier#recommended-configuration)
  ...
  - Run the prettier script (`prettier --write`) _before_ the ESLint script (`eslint --fix`) or else you'll get issues when the prettier format conflicts with your ESLint rules.
    - A package called [prettier-eslint](https://github.com/prettier/prettier-eslint) will do these this for you (running Prettier then ESLint) but it's easy enough to do it yourself.

...
export function getFileBySlug(
  type,
  slug,
  fields = { content: false, metadata: true }
) {
  ...
  const lastmod = metadata.lastmod
    ? new Date(metadata.lastmod).toLocaleDateString()
    : date;
  let returnObj = {
...
export function getFilesByType(
  ...
  const notes = slugs
    ...
    .sort((file1, file2) =>
      new Date(file1.metadata?.date).toISOString() <
      new Date(file2.metadata?.date).toISOString()
        ? 1
        : -1
    );
  return notes;
